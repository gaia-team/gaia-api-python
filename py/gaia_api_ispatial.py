...
# Avro Object Definitions location
obj_defs_path = os.path.join(os.path.dirname(__file__), "../obj_defs")
...
def read_orig_datum(SCHEMA, encoded_datum, encoding = 'BINARY'):
    if encoding == None:
        encoding = 'BINARY'
    ...
    if encoding == 'BINARY':
        ...
        elif not isinstance(SCHEMA, schema.Schema):
            raise TypeError('Unable to parse Avro Schema')
...
def read_datum(SCHEMA, encoded_datum):
    ...
    REP_SCHEMA_STR = open(obj_defs_path + "/gaia_response.json","r").read()
    ...
    if stype == 'none':
        ...
        DATA_SCHEMA_STR = open(obj_defs_path + "/%s.json"%(stype), "r").read()
...
def read_big_point(encoded_datum,encoding=None):
    ...
    OBJ_SCHEMA = schema.parse(OBJ_SCHEMA_STR)
    return read_orig_datum(OBJ_SCHEMA, encoded_datum, encoding)
...
def get_schemas(base_name):
    REP_SCHEMA_STR = open(obj_defs_path + "/"+base_name+"_response.json", "r").read()
    REP_SCHEMA = schema.parse(REP_SCHEMA_STR)
    REQ_SCHEMA_STR = open(obj_defs_path + "/"+base_name+"_request.json", "r").read()
...
def do_register_type(type_def, type_annot=None):
    """Registers a new Avro schema with GAIA.

    If the schema is already registered, the existing type ID is reused.
    Optionally, you may assign a type annotation to a schema.
    """
    if not type_annot:
        type_annot = ''
    datum = {
...
def do_register_type_point():
    ...
    return do_register_type("""{"type":"record","name":"point","fields":[{"name":"x","type":"double"},{"name":"y","type":"double"}]}""")
...
def do_register_type_big_point():
    ...
    return do_register_type("""{"type":"record","name":"point","fields":[{"name":"msg_id","type":"string"},{"name":"x","type":"double"},{"name":"y","type":"double"},{"name":"timestamp","type":"int"},{"name":"source","type":"string"},{"name":"group_id","type":"string"}]}""",
                            """msg_id""")

def do_register_type_gis_point():
    # NOTE: should we use a int or a long for the timestamp? we can't use double because that will expect a decimal value
    #jsonobj = '{"type_definition":"{\\"type\\":\\"record\\",\\"name\\":\\"point\\",\\"fields\\":[{\\"name\\":\\"msg_id\\",\\"type\\":\\"string\\"},{\\"name\\":\\"x\\",\\"type\\":\\"double\\"},{\\"name\\":\\"y\\",\\"type\\":\\"double\\"},{\\"name\\":\\"timestamp\\",\\"type\\":\\"int\\"},{\\"name\\":\\"source\\",\\"type\\":\\"string\\"},{\\"name\\":\\"group_id\\",\\"type\\":\\"string\\"}]}"}'
    REP_SCHEMA_STR = open(obj_defs_path + "/register_type_response.json", "r").read()
    REP_SCHEMA = schema.parse(REP_SCHEMA_STR)
    SCHEMA_STR = open(obj_defs_path + "/register_type_request.json", "r").read()
...
#get set
def do_get_set(set_id, start, end, user_auth=""):
...
def do_new_set(type_id, set_id):
    ...
    REP_SCHEMA_STR = open(obj_defs_path + "/new_set_response.json", "r").read()
    REP_SCHEMA = schema.parse(REP_SCHEMA_STR)
    #SCHEMA_STR = """{"type":"record","name":"new_set_request","fields":[{"name":"set_id","type":"string"},{"name":"type_id","type":"string"}]}"""    
    SCHEMA_STR = open(obj_defs_path + "/new_set_request.json", "r").read()
    SCHEMA = schema.parse(SCHEMA_STR)
...
def dict_to_datum(type_def, source):
    """Marshalls a dictionary into a type definition."""
    if isinstance(type_def, (str, unicode,)):
        type_def = schema.parse(type_def)
    elif not isinstance(type_def, schema.Schema):
        raise TypeError('Cannot convert dictionary to non-Schema')
    return write_datum(schema, source)

def do_add_datum(set_id, type_def, source):
    (REQ_SCHEMA,REP_SCHEMA) = get_schemas("add_object")
    
    OBJ_SCHEMA = schema.parse(type_def)
    jsonobj = write_datum(REQ_SCHEMA,
                          {
                            'set_id': set_id,
                            'object_data': write_datum(OBJ_SCHEMA,
                                                       source),
                            'object_data_str': '',
                            'object_encoding': 'BINARY'
                          })
...
def do_add_point(set_id, x, y):
    ...
    return do_add_datum(set_id, OBJ_SCHEMA_STR, datum)
...
#equi-join
def do_join(left_set, left_attr, right_set, right_attr, result_type, result_set, user_auth=""):
    # get the schemas
...
#exit
def do_exit(exit_type, authorization):
...
def do_copy_set(new_set_id, original_set_id, selector, type_id, user_auth=""):
...
# cluster
def do_cluster(world_set, subworld_set, result_set, shared_attribute, cluster_attribute, user_auth=""):
...
def do_join_setup(left_set, left_attr, right_set, right_attr, subset_id, user_auth=""):
    # get the schemas
...
# join incremental
def do_join_incremental(left_subset, left_attr, left_index, right_set, right_attr, result_set, result_type, user_auth=""):
    # get the schemas
...
#get set sorted
def do_get_set_sorted(set_id, start, end, user_auth=""):
...
def do_read_trigger_msg(encoded_datum):
    REP_SCHEMA_STR = open(obj_defs_path + "/trigger_notification.json", "r").read()
    REP_SCHEMA = schema.parse(REP_SCHEMA_STR)

    return read_orig_datum(REP_SCHEMA, encoded_datum, encoding='JSON')
